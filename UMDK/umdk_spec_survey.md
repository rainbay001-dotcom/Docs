...
- **Bojie Li, APNet'21 keynote summary** at [01.me](https://01.me/en/2023/09/towards-compute-native-networking/) (Sept 2023) — earlier framing; mentions UBMMU integrated into CPUs and an Orchestrator engine. The 2023 talk uses URMA address format `(Entity ID, UASID, offset)` while the current spec uses UBMD = `(EID, TokenID, UBA)`. **CONFIRMED via Chinese-spec read 2026-04-25**: UASID was renamed to TokenID; offset is now part of UBA inside the UBMD 3-tuple. See [`umdk_spec_deep_dive.md`](umdk_spec_deep_dive.md) §3.4 (EID format) and §4 (token mechanics including the §11.4.4 rotation protocol).
...
- **CloudMatrix384 paper** ([arXiv:2506.12708](https://arxiv.org/abs/2506.12708)) — first peer-reviewed UB benchmarks: prefill 6,688 tok/s/NPU and decode 1,943 tok/s/NPU at <50 ms TPOT on DeepSeek-R1. Measured BW: 196 GB/s NPU↔NPU, 1.2 µs latency at 512 B. Three-plane network (UB / RoCE / VPC) per node. Software stack uses CANN/HCCL — does **not** name URMA. Deep read in [`umdk_academic_papers.md`](umdk_academic_papers.md) §2.
- **UB-Mesh paper** ([arXiv:2503.20377](https://arxiv.org/abs/2503.20377)) — Huawei research preprint on nD-FullMesh topology atop UB. UB-Mesh-Pod = 1,024 NPUs (4D-FullMesh), 64+1 backup per rack, All-Path-Routing with 8-byte source-routing header + 2-VL deadlock-free flow control. **2.04× cost-efficiency**, **98.8% availability** (vs 91.6% Clos), **95%+ training linearity** to 64K NPUs. 86.7% of cables are passive electrical. Deep read in [`umdk_academic_papers.md`](umdk_academic_papers.md) §1.
