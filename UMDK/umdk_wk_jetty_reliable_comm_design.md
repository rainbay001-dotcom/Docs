...
static void ubmad_wk_syn_rt_work_handler(struct work_struct *work)
{
    ...
    if (session->rt_cnt >= UBMAD_WK_MAX_RETRY) {
        ...
        case UBMAD_WK_LAST_ACK:
            /* v3.15 新增：FIN/FIN_ACK 重传超限。alloc ref 已经在
             * ubmad_wk_close consume，上层早已不持 session 指针。再调
             * on_closed 给它一个"借用指针"是危险的：listener 的通用
             * on_closed 实现会按 §6.5 调 ubmad_wk_close，→ close 又
             * consume 一份 ref → double-put + UAF。所以这条路径只
             * log，不通知。Session 仅剩 timer ref，下面统一释放。
             * §18.8 已扩展涵盖"close-side 失败也无回调"。 */
            pr_info("ubmad_wk: session=%u FIN/FIN_ACK retry exhausted in "
                    "state=%d; no upper notification (caller already "
                    "released session pointer via close)\n",
                    session->session_id, state);
            break;
...
**副作用**：上层无法感知 (a) accept 失败、(b) close 路径上 FIN/FIN_ACK 投递最终失败。运维和故障诊断可能需要这两个信号。
