...
_文档版本：v3.25（基于内核源码探索新增两节：§18.9 UBcore 事件接入加速链路故障检测；§20 Linux TCP 三次握手与链路故障处理参考对照）_
_最后更新：2026-05-14_
...
事件类型 `enum ubcore_event_type`（`include/ub/urma/ubcore_types.h:357-376`，共 16 项）：
...
- UDMA 链路状态变化 → `PORT_DOWN/PORT_ACTIVE`（`udma_main.c:1065-1083`，物理 link 真的 up/down 时触发）
- UDMA 控制路径错误 → `TP_ERR`、`TP_FLUSH_DONE`（`udma_ctl.c:1075-1078`）
...
- **方案 A（推荐）**：扩展现有 `ubmad_event_cb`，按事件类型分派：
  - `PORT_DOWN` / `DEV_FATAL` → 硬杀：遍历 `g_session_hash`，找到所有 `session->dev_priv == container_of(handler, ubmad_device_priv, handler)` 的 session，每个标 `state=UBMAD_WK_CLOSED`、`result=-ENETDOWN`，按 §7.2 retry-limit 的"external owner notifies; internal release otherwise"四态 switch 分别处理通知和 ref 释放。
  - `TP_ERR`（per-TP）→ 范围更窄：只命中 vTP 路由经过该 TP 的 session。需要按 `event->element.tp` 反查或按 EID 二级匹配。
  - `TP_SUSPEND` → 软信号：标 session 进"加速重试"模式（如把下次 rt_work delay 立刻改到最小值），但不立即 close——挂起可能是瞬时的。
  - `TP_FLUSH_DONE` → 通常是 close 后的清理同步信号，可作为 release_session 加速契机但非必需。
...
**预期检测延迟**：物理 link-down 事件 → workqueue 调度 → callback → 微秒~毫秒级（取决于 workqueue 负载）。比 v1 的 ~4s `MAX_RETRY` 窗口快 3-4 个数量级。
