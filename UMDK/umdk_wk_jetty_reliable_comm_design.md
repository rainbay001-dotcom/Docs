...
_文档版本：v3.14（修正 v3.13 引入的 on_closed 所有权歧义：SYN_RCVD 失败时不调 on_closed，避免上层 close consume 触发 double-put）_
_最后更新：2026-05-14_
...
static void ubmad_wk_syn_rt_work_handler(struct work_struct *work)
{
    ...
    if (session->rt_cnt >= UBMAD_WK_MAX_RETRY) {
        ...
        if (state == UBMAD_WK_SYN_RCVD) {
            /* v3.14 修订：服务端 pre-established 失败。**不要**调 on_closed——
             * 这个 session 从未通过 on_established 移交给上层，上层既没有
             * 它的指针、也不"拥有"它。但 session->on_closed 是从 listen
             * session 复制来的通用回调，按 §6.5 文档化的被动关闭模式，
             * 上层在 on_closed 里典型地会调 ubmad_wk_close(session)。
             * 而 ubmad_wk_close 即使在 invalid/CLOSED 状态也会 consume
             * 调用方的 ref（v3.6 close-consume 语义）。如果 v3.13 那样调
             * on_closed → 上层调 close → close 又消费一次 alloc ref →
             * 紧接着本 handler 的 alloc-ref put 命中已释放内存：double-put + UAF。
             *
             * 正确做法：SYN_RCVD 失败时不通知上层，只在内部 log 并释放
             * alloc ref。代价是上层无法感知"某个 accept 失败了"——这是 v1 的
             * 已知限制（§18 列出"无 listen 失败回调"待 v2 解决）。
             *
             * v3.13 修订（仍然适用）：alloc 拿到的那 1 份 ref 从未移交给
             * 上层，必须在此处释放，否则 session 永远 leak。其它三种状态
             * 不需要这个 alloc-put：
             *   - SYN_SENT：alloc ref 由 connect() 调用方持有。
             *   - FIN_WAIT_1 / LAST_ACK：alloc ref 已经在 ubmad_wk_close
             *     consume，此时 session 上只剩 timer ref。 */
            ...
            /* SYN_SENT (client connect): wakes caller via on_closed 或 complete。
             * FIN_WAIT_1 / LAST_ACK: 通知上层关闭侧超时。
             * 三种状态下 alloc ref 都由外部持有者（connect 调用方或上层），
             * 上层调 ubmad_wk_close 时 consume，不在此处释放。 */
            if (session->on_closed)
...
### 18.8 [MEDIUM] 服务端 listen 没有"accept 失败"通知机制（v3.14 引入）

**问题**：v3.14 修订让 SYN_RCVD 重传超限时**不**调用 on_closed——因为该 session 从未通过 on_established 移交给上层，上层既没有指针也不"拥有"它，调 on_closed 会让上层按"被动关闭"模式调 ubmad_wk_close 而 double-put / UAF（详见 §7.2 retry-limit 分支注释）。

**副作用**：服务端上层无法感知"某次 accept 因为客户端不发 ACK 而失败"。这类信息可能对运维（统计失败率）或安全（识别扫描）有用。
...
- 或者更通用的事件流（perf_event / netlink notify）让 listener 订阅。
