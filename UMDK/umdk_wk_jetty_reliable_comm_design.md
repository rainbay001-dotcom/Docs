...
_文档版本：v3.28（§20 大扩展：从 ~150 行 6 子节扩到 ~400 行 17 子节，覆盖架构 / 数据结构 / 12 状态 / 发送 / 接收 / 握手 / 挥手 / 计时器 / 拥塞控制 / 丢包恢复 / 流控 / ACK / 哈希表 / netdev / 链路故障 / 对照 / 经验映射）_
_最后更新：2026-05-15_
...
17. [关键数据流图](#关键数据流图)
18. [未决设计问题](#未决设计问题)
19. [评审过程回顾](#19-评审过程回顾v324-新增)
20. [参考：Linux TCP 三次握手与链路故障处理](#20-参考linux-tcp-三次握手与链路故障处理v325-新增)
...
struct ubmad_wk_session {
    ...
    /* session 控制消息重传定时器：SYN / SYN_ACK / FIN（v3.2 起兼任 close-side
     * FIN 重传，按 state 在 handler 的 switch 内分派）。**FIN_ACK 不走此路径**——
     * FIN_ACK 是被 process_wk_fin() 在 CLOSE_WAIT/TIME_WAIT 收到重传 FIN 时
     * 反应式重发的，不由本 timer 主动驱动。 */
...
static void ubmad_wk_syn_rt_work_handler(struct work_struct *work)
{
    ...
    if (session->rt_cnt >= UBMAD_WK_MAX_RETRY) {
        ...
        /* v3.15 修订：按"调 close 之前 vs 之后"区分 alloc ref 所有者。
         * v3.6 close-consume 语义意味着只要调用方调过 ubmad_wk_close，
         * 它就不再持有 session 指针；此时 on_closed(session, ...) 的
         * session 是"借用指针"，调用方不能再用——典型 listener 实现按
         * §6.5 模式调 ubmad_wk_close 会 double-put + UAF。
         *
         * 四种 retry-relevant 状态的所有权差异：
         *   SYN_SENT       : 调用方还没调 close（connect 同步等或异步收回调），
         *                    持 alloc ref → 通知合法。
         *   SYN_RCVD       : 上层从未通过 on_established 拿到，持 alloc ref
         *                    的是内部代码 → 内部释放，不通知。
         *   FIN_WAIT_1     : 调用方已调 close（→ FIN_WAIT_1），alloc ref 已
         *                    consume，上层不再持指针 → 不通知。
         *   LAST_ACK       : 同 FIN_WAIT_1（被动方 CLOSE_WAIT → LAST_ACK
         *                    走的是同一个 close）。 */
        switch (state) {
        ...
        case UBMAD_WK_SYN_RCVD:
            /* Server pre-accept：内部释放 alloc ref，不通知上层。
             * v3.14 修订留下的 §18.8（"v1 没有 accept 失败回调"）依然适用。 */
            pr_info("ubmad_wk: pre-established session=%u timed out in "
                    "SYN_RCVD before final ACK; no upper notification\n",
                    session->session_id);
            ubmad_put_session(session);   /* alloc ref */
            break;

        case UBMAD_WK_FIN_WAIT_1:
        case UBMAD_WK_LAST_ACK:
            /* v3.15 新增：FIN 重传超限——也即对端始终没回 FIN_ACK。
             * （注意：本 timer 只重传 FIN；FIN_ACK 是 process_wk_fin()
             * 在 CLOSE_WAIT/TIME_WAIT 收到重传 FIN 时反应式重发，不在
             * 本 timer 范围。）
             *
             * alloc ref 已经在 ubmad_wk_close consume，上层早已不持
             * session 指针。再调 on_closed 给它一个"借用指针"是危险的：
             * listener 的通用 on_closed 实现会按 §6.5 调 ubmad_wk_close
             * → close 又 consume 一份 ref → double-put + UAF。所以这条
             * 路径只 log，不通知。Session 仅剩 timer ref，下面统一释放。
             * §18.8 已扩展涵盖"close-side 失败也无回调"。 */
            pr_info("ubmad_wk: session=%u FIN retry exhausted (no FIN_ACK "
                    "received) in state=%d; no upper notification "
                    "(caller already released session pointer via close)\n",
                    session->session_id, state);
            break;
...
| 13 | `ubmad_wk_syn_rt_work_handler()` | `ubmad_datapath.c` | session 控制消息重传工作（**SYN / SYN_ACK / FIN**——FIN_ACK 不在此 timer 范围，由 process_wk_fin 反应式重发）；指数退避；超限按 v3.15 四态 switch 处理——仅 SYN_SENT 通知调用方（on_closed 或 complete），SYN_RCVD 内部释放 alloc ref + 不通知，FIN_WAIT_1/LAST_ACK 仅 log + 不通知（alloc ref 已被 close consume）|
...
**解决**（v3.20-v3.21 起）：本设计已经在所有 send 路径上覆盖了三种自愈机制，多数情况无需手动干预：

1. **caller pre-import**——`ubmad_wk_connect / process_wk_syn / wk_close` 在第一次 send 之前同步调用 `ubmad_import_jetty()`（§4.4 / §5.4 / §6.2）。
2. **`ubmad_post_send_wk_with_import()` 反应式包装**——`process_wk_syn_ack`/`process_wk_fin` 的所有 ACK / FIN_ACK 应答路径用这个 helper，撞 `-EAGAIN` 自动同步 import + retry once（§4.3 +）。
3. **`ubmad_wk_syn_rt_work_handler` import-on-EAGAIN 兜底**——重传路径在看到 `-EAGAIN` 时也会同步 import（§7.2）。

如果依然撞到 `-EAGAIN` 持续不消，常见根因是对端设备真的不在线（`ubmad_import_jetty` 内部的 UBCM 协商超时返回 `-EHOSTUNREACH`）；用 `dmesg | grep "with_import: import failed"` 或 `"re-import tjetty failed"` 定位。
...
### 18.8 [MEDIUM] retry-exhaust 失败没有"无 session 指针"的通知机制（v3.14/v3.15 扩大）

**问题**：v3.14 + v3.15 修订让以下两类 retry-exhaust 失败时**不**调用 on_closed，因为这些状态下 session 指针不属于上层（详见 §7.2 retry-limit 分支注释）：

- **SYN_RCVD**（v3.14）：服务端在客户端发 ACK 之前 retry 超限。session 从未通过 on_established 移交给上层。
- **FIN_WAIT_1 / LAST_ACK**（v3.15）：主动方/被动方在对端发 FIN_ACK 之前 retry 超限。alloc ref 已经在 ubmad_wk_close 时 consume，调用方按 close-consume 语义早已不持 session 指针。

调 on_closed 给一个上层不持有的 session 指针风险是：listener 的通用 on_closed 实现典型地会按 §6.5 模式调 `ubmad_wk_close(session)`；v3.6 close-consume 在 invalid/CLOSED 状态依然消费一份 ref → double-put + UAF。所以这两类失败只在内部 log。

**副作用**：上层无法感知 (a) accept 失败、(b) close 路径上 FIN 重传超限（即对端始终没回 FIN_ACK）。运维和故障诊断可能需要这两个信号。

**v1 处理**：纯在内部 log，上层不感知。

**待定（v2 候选 API）**：
- 给 `ubmad_wk_listen()` 加一个独立的 `on_failed(union ubcore_eid *peer_eid, int error, void *ctx)` 回调，专门用于通知 pre-established 失败。回调签名故意不带 session 指针（因为根本没法安全地交出来）。
- 给主/被动方提供一个对应的 close 失败回调（同样 no-session-pointer 签名），或者把两者合并成一个 `on_session_event(eid, event_type, error, ctx)`。
- 或者更通用的事件流（perf_event / netlink notify）让上层订阅。

**影响**：纯通知接口完整性，不影响 v1 的核心可靠性语义。

### 18.9 [HIGH for v2] 接入 UBcore TP/Port 事件以加速链路故障检测

**问题**：v1 的故障检测完全依赖 `UBMAD_WK_MAX_RETRY` 重传超限——上限约 4 秒（见 §0.3 MSN 窗口算式）。Linux TCP 在同等场景下实际上可能要拖到 ~20 分钟（默认 `tcp_retries2 = 15`），所以 4 秒已经远好于通用 TCP；但 UBcore 自己有更快的信号源。

**已存在的 UBcore 事件基础设施**：

事件类型 `enum ubcore_event_type`（`include/ub/urma/ubcore_types.h:357-376`，**共 18 项**）：

```c
UBCORE_EVENT_PORT_DOWN          ← 物理端口 down
UBCORE_EVENT_PORT_ACTIVE        ← 物理端口恢复
UBCORE_EVENT_DEV_FATAL          ← 整个设备死掉
UBCORE_EVENT_TP_ERR             ← 单个 TP 出错
UBCORE_EVENT_TP_SUSPEND         ← 单个 TP 暂时挂起（可恢复）
UBCORE_EVENT_TP_FLUSH_DONE      ← TP flush 完成（清理同步）
UBCORE_EVENT_EID_CHANGE         ← EID 表变化
UBCORE_EVENT_J{FC,FS,FR,ETTY,ETTY_GRP}_ERR / *_LIMIT_REACHED
UBCORE_EVENT_ELR_{ERR,DONE}, UBCORE_EVENT_MIGRATE_VTP_{SWITCH,ROLLBACK}
```

事件结构（`ubcore_types.h:1250-1264`）按类型携带不同的 element 联合：`port_id`（PORT_*）、`tp`（TP_*）、`vtp`（VTP_*）、`jetty/jfc/jfr/jetty_grp`（对象级错误）等。

**两条 dispatch 路径**——区分清楚：

1. **设备级 `event_handler_list`**：`PORT_*`、`TP_*`、`DEV_FATAL`、`EID_CHANGE` 走这条。注册 API `ubcore_register_event_handler(dev, handler)` (`ubcore_device.c:1358`)；触发 API `ubcore_dispatch_async_event(event)` (`:1431`) → workqueue → `ubcore_dispatch_event_clients()` (`:1377`) 遍历列表逐个 callback。
2. **per-jetty `jetty->jfae_handler`**：仅对**对象级**错误（`JFC_ERR/JFS_ERR/JFR_ERR/JETTY_ERR/JETTY_GRP_ERR/*_LIMIT_REACHED`）触发，由 HW EQ 直接调用（如 `udma_eq.c:114,130,165,200,233`）。**PORT_DOWN/TP_ERR/DEV_FATAL 不走这条**——这是设计 v2 时容易踩的坑。

**已确认的 producer（v3.26 修订：实际 grep `ubcore_dispatch_async_event` 调用点）**：

| 事件 | Producer | 触发场景 |
|---|---|---|
| `UBCORE_EVENT_PORT_DOWN` / `PORT_ACTIVE` | `udma_main.c:1065-1083` | UDMA 物理链路 up/down |
| `UBCORE_EVENT_ELR_DONE` / `ELR_ERR` | `udma_main.c:1175-1179` | UDMA reset 阶段进展 |
| `UBCORE_EVENT_EID_CHANGE` | `ubcore_uvs_cmd.c:101`、`ubcore_genl_admin.c:254`、`ubcore_device.c:1574,2191` | EID 表条目增删/更新 |

**v3.25 曾错误地把 `udma_ctl.c:1075` 列为 `TP_ERR/TP_FLUSH_DONE` 的 producer——那段实际上是 `to_hw_ae_event_type` 转换函数，把 `UBCORE_EVENT_*` 翻译成 `UBASE_EVENT_TYPE_*` 给 AE auxiliary-info 查询用，是消费侧映射，不是真实 producer。**

**已枚举但没有 producer 的事件**（同 §4.8/§4.9/§4.12 文档化的"dormant scaffolding"模式）：

- `UBCORE_EVENT_TP_ERR / TP_SUSPEND / TP_FLUSH_DONE` ——本设计列举的 TP 级事件**目前无 dispatch 触发点**，只在 `udma_ctl.c` 的转换 switch 和事件结构定义里出现。
- `UBCORE_EVENT_DEV_FATAL` ——唯一一处出现是 `uburma_uobj.c:1287` 通过 `uburma_write_event` 写到用户态事件通道，不经过 `ubcore_dispatch_async_event`；意味着 listener 用 `register_event_handler` 也收不到这个事件。
- `UBCORE_EVENT_J{FC,FS,FR,ETTY,ETTY_GRP}_ERR / *_LIMIT_REACHED`：这些是 per-jetty 错误，由 HW EQ 直接触发 `jetty->jfae_handler`（如 `udma_eq.c:114,130,165,200,233`），不进 device 级 `event_handler_list`。
- `UBCORE_EVENT_MIGRATE_VTP_{SWITCH,ROLLBACK}`：仅枚举定义，无 producer。

**对本设计的影响**：v1 能依赖的"快速链路故障检测"信号目前只有 **PORT_DOWN/PORT_ACTIVE** 和 **ELR_DONE/ELR_ERR**——粒度是设备级（一个 UDMA device 一个 port）。TP 级精细化检测和 device-fatal 通知需要等上游补 producer（或本设计自己在 UBcore 加 dispatch 调用，但那是另外的工作）。即使如此，PORT_DOWN 一项就足以让链路插拔/故障的 session 从 ~4s `MAX_RETRY` 检测降到事件级——这是 §18.9 的主要价值。

**UBMAD 现有 handler**：`ub_mad.c:1124-1125` 已经用 `ubcore_register_event_handler` 注册了 `dev_priv->handler`。但 callback `ubmad_event_cb`（`ub_mad.c:134-146`）目前几乎是 no-op：

```c
switch (event->event_type) {
case UBCORE_EVENT_EID_CHANGE: ubcore_log_info(...); break;
default: ubcore_log_err("Invalid event_type: %d ...");  break;
}
```

PORT_DOWN/TP_ERR/DEV_FATAL 通通落到 `default` 分支被当成"非法事件"丢掉——**基础设施已就位，消费者是空架子**。

**两种集成方案**：

- **方案 A（推荐）**：扩展现有 `ubmad_event_cb`，按事件类型分派：
  - `PORT_DOWN`（**有 producer**）→ 硬杀：遍历 `g_session_hash`，找到所有 `session->dev_priv == container_of(handler, ubmad_device_priv, handler)` 的 session，每个标 `state=UBMAD_WK_CLOSED`、`result=-ENETDOWN`，按 §7.2 retry-limit 的"external owner notifies; internal release otherwise"四态 switch 分别处理通知和 ref 释放。**这是 v1/v2 实际能拿到信号的主路径**。
  - `PORT_ACTIVE`（**有 producer**）→ 仅 log，**不**自动恢复 close 掉的 session（同 TCP 不重连）；上层负责重发起 connect。
  - `ELR_DONE` / `ELR_ERR`（**有 producer**）→ UDMA reset 阶段事件；语义上等同于 device 进入/退出 reset 流程。`ELR_ERR` 当作 `DEV_FATAL` 的等价处理（硬杀所有 session）；`ELR_DONE` 当 `PORT_ACTIVE` 处理（仅 log）。
  - `EID_CHANGE`（**有 producer**）→ 已有处理（保留 v1 行为，仅 log）。
  - `DEV_FATAL` / `TP_ERR` / `TP_SUSPEND` / `TP_FLUSH_DONE`（**目前无 producer**）→ **防御性代码**：分支留 case 但只 log。如果未来上游补了 producer，可以按 (DEV_FATAL → 硬杀 device 全 session)、(TP_ERR → 按 `event->element.tp` 反查命中的 session 硬杀)、(TP_SUSPEND → 软信号触发加速重试)、(TP_FLUSH_DONE → release_session 加速契机) 的方向扩展。
- **方案 B**：单独 `session_event_handler` 注册，与 UBMAD 自身的 `dev_priv->handler` 解耦。每事件多遍历一次列表（开销极小），但代码隔离更清晰。

推荐方案 A：UBMAD 已经持有 `dev_priv->handler`，扩展它符合"一个上层组件一个 handler"的约定，且共享 dev_priv 上下文便于反查 device 范围内的 session。

**预期检测延迟（v3.26 修订：限定到实际有 producer 的事件）**：

| 故障场景 | v1 检测路径 | v1 检测时间 | 加 §18.9 后 |
|---|---|---|---|
| 物理 link 拔线 / interface down | `MAX_RETRY` 重传超限 | ~4s | PORT_DOWN 事件 → workqueue → callback → 微秒~毫秒级 |
| UDMA 设备 reset / 故障 | `MAX_RETRY` 重传超限 | ~4s | ELR_ERR 事件 → 同上 |
| 对端进程崩溃但本端 link OK | `MAX_RETRY` 重传超限 | ~4s | **同 v1，无加速**——这类事件 UBcore 现在不报 |
| 对端 NIC 故障但本端 link OK | `MAX_RETRY` 重传超限 | ~4s | **同 v1**——本端的 PORT 没事，只是 TP 端到端断了 |
| 单个 vTP/TP 的硬件错误 | `MAX_RETRY` 重传超限 | ~4s | **理论上**靠 TP_ERR，但目前无 producer——同 v1 |

也就是 §18.9 的实际收益**主要在本端 link/device 故障**这个具体的子集上；对端故障还是只能靠 `MAX_RETRY` 兜底。这对很多场景仍然有用（运维拔线、网卡硬件故障、设备 reset），但不要过度承诺为"全场景事件级故障检测"。

**注意事项**：
- `ubcore_dispatch_async_event` 是 workqueue 异步，不在事件源上下文执行，**callback 内可以 sleep**——但要避免持有 `g_session_hash_lock` 时阻塞。建议先在锁内收集要 abort 的 session 引用列表，解锁后逐个推进状态机。
- 与 §7.4 ref-counting invariant 对齐：每个被强制 close 的 session 需要按"external owner notifies; internal release otherwise"四态分派——`SYN_RCVD` 是 internal owner（内部释放 alloc ref），其它三态的 alloc ref 在外部，按 v3.15 §7.2 retry-limit 模式通知。
- PORT_ACTIVE 事件**不要**自动恢复 close 掉的 session——TCP 也不这么做（Linux 不重连）；上层负责重发起 connect。

**待定**：
- 事件 → session 反查的索引：v1 单 listen 时全表扫够用；高并发场景需要 (dev_priv → session 列表) 索引。
- TP_SUSPEND 的"加速重试"策略：是把 rt_cnt 重置还是 delay clamp 到 0？前者保留指数退避语义、后者更激进。
- 是否暴露 `UBMAD_WK_FAULT_DETECT_FAST` 配置项让用户在"慢但稳"（仅 retry 超限）和"快但响应抖动"（事件触发）之间二选一。

**影响**：把 v1 的"4 秒级故障检测"提升到"事件级故障检测"。是 v2 的高优先级增强项；不是 v1 必须，但实现成本低（一个 callback + 一次哈希遍历），收益显著。

---

## 19. 评审过程回顾（v3.24 新增）

> 这一节是元内容——记录 v2 → v3.23 的评审迭代过程，以及途中收敛到的几条工程经验。供后续读者理解为什么本文档有这么多版本注释，以及给未来类似的多状态 / ref-counting / 协议设计文档的评审提供参考。

### 19.1 数字摘要

- **本文档：** 23 个迭代版本（v2 baseline + v3.0 → v3.23），最终约 3000 行。
- **同步配套文档：** `umdk_urma_jetty_kernel_call_trace.md` 的 §4.10 重写（3-plane UAPI）+ §4.11（bonding flow）+ §4.12（UBMAD jetty ID 2 dead-coded）+ §8 三处 sync。
- **每轮 review 发现的问题数量序列**：
  ```
  7 → 6 → 4 → 4 → 3 → 2 → 2 → 1 → 0+3 → 2 → 1+1 → 1 → 1 → 1
   → 0+2 → 0+2 → 0+3 → 1H → 1H → 0+3 → 0+1 → 0+1 → 1L → 0
  ```
  （格式：`N` = 总问题数，`+M` = 其中 prose-only，`H/L` = HIGH/LOW 单一严重度）
- **问题分类：**
  - 协议 / wire-format 错（1，v3.2 单 session_id 的双侧握手永远完不成）
  - kref / lifetime UAF / leak（约 5 处）
  - API / 所有权语义歧义（约 2 处）
  - 反应式 send 的错误恢复缺口（约 2 处）
  - prose drift（约 15 处）
  - 术语用词不准（约 5 处）

### 19.2 收敛性观察

- **「No findings」 ≠ 「Doc is bug-free」**：直到 round 22-23 才出现连续两轮零问题。早期偶发的"零代码 bug"轮（round 9、round 15）紧接着就被新一轮结构改动（v3.9 invariant rewrite、v3.15 4-state switch）引入的二级失败模式打破。**真收敛信号是连续两轮 clean。**
- **潜伏 bug 比想象的久**：v3.12 修的 LAST_ACK double-put UAF 自 v3.2 就在代码里。v3.9 的 invariant 重写让那个曾经"凑巧平衡"的 over-put 变成了真 UAF。从 bug 写入到被发现 ≈ 11 轮 review。reviewer 不会保证每轮都全文扫一遍，他们扫到哪段、用什么视角看，决定了发现什么。

### 19.3 几条结构化经验（值得记住的）

1. **「External owner notifies; internal release otherwise」+ 4-态显式 switch + `WARN_ON_ONCE`** 是终结 kref/lifetime bug cluster 的关键结构改动。在 v3.5-v3.7 那几轮一直在打补丁，到 v3.15 才收敛到这个明确表述；但事后看，这套规则**应该是 v3 一开始就该有的架构**，不是等出 bug 再补的事后规范。
2. **集中式错误恢复 helper（如 `ubmad_post_send_wk_with_import`）+ 一致地应用到 ALL 同类 call site**，比一处一处加 inline 处理更能避免"修了这处忘了那处"。round 18 修了 3 处 send 路径的 cache miss 恢复，round 19 reviewer 又指出 5 处缺的；round 21 的 helper 才把这一类彻底闭合。**模式：发现一类问题时，列出该类的所有 call site，统一改 / 包装，而不是只修触发 review 的那几处。**
3. **Self-documenting code-changelog 文档的稳态成本是 prose drift。** 每次实质代码改动牵涉至少 5-10 处描述同步：函数 header 注释、§ 章解释、ref-counting 表、function summary 表、version changelog 块。我习惯只更新前 1-2 处；reviewer 在后续 1-3 轮才捕到剩下的。如果不能一次同步完，至少要在改动 PR 说明里列清楚"本次预期同步的描述位置 N 处"作为自检 checklist。
4. **Diary 里写下来的 "next round 应该 self-audit X" 几乎都没自己去做**，每次都是 reviewer 在下一轮发现了我预测会发现的东西。**写下来的 self-audit 该当场做完，否则等同于把工作量推给 reviewer。**
5. **Reviewer 没找代码 bug 的轮次一定要警惕——可能只是该轮没看那个角度。** 真正 production-ready 的信号是 (a) 连续两轮零问题，(b) 同时有别的高保真信号（编译通过、跑通测试），二者缺一不可。

### 19.4 当前状态与剩余工作

到 v3.23 / round 23 为止，doc-iteration 能闭环的部分都闭环了。仍然不能由 review 解决的两类工作：

- **§18 设计问题** — 数据平面、v1 消费者、多 listen ABI 等需要 stakeholder 输入，不是 reviewer 视角。
- **实际编译 / 运行验证** — 没有人在真实内核上跑过这份代码。这是 doc-iteration 完全无法验证的层面；找一台 NPU 服务器编译进 ubcore 模块、跑端到端握手 + 挥手 + 故障注入测试，会比再来 24 轮 doc review 更能暴露剩余 bug。

如果第二阶段（actual implementation）发现了新 bug，可以在 §19.5 续记。

---

## 20. 参考：Linux TCP 架构深潜与可靠通信设计对标（v3.28 扩展）

> 本节是参考材料——把 Linux TCP（OLK-6.6 内核）的完整架构、核心数据结构、状态机、收发路径、握手与挥手流程、计时器、拥塞控制、故障检测、socket 哈希表管理的关键 file:line 列出来。供本设计的实现者和评审者对照。本设计有意比 TCP 简化（单设备、固定 demux、无拥塞控制），但 TCP 已在生产验证过的几条工程模式（两阶段 alloc / SYN cookie / 快速故障信号 / 可调超时）是本设计 v2 可吸收的。v3.25 起此节存在；v3.28 从单一握手 + 链路故障描述扩展为 16-子节深度参考。

### 20.1 TCP 栈分层架构与子模块速查

```
┌────────────────────────────────────────────────────┐
│ APP                sendmsg(2) recvmsg(2) close(2) │
└────────────────────┬───────────────────────────────┘
                     ↓
┌────────────────────────────────────────────────────┐
│ Socket API     net/socket.c → sock_sendmsg/recvmsg │
└────────────────────┬───────────────────────────────┘
                     ↓
┌─────────────────────────────────────────────────────────┐
│ TCP send 路径                  TCP recv 路径            │
│ tcp_sendmsg / tcp_output.c ←→  tcp_input.c              │
│ Nagle/GSO/重传/CC/窗口      ←→  序列号/RTT/SACK/乱序     │
└────┬────────────────────────────────┬───────────────────┘
     ↓                                ↓
┌─────────────┐               ┌─────────────────────┐
│ Timers      │               │ Hash tables         │
│ tcp_timer.c │               │ inet_hashtables.c   │
│ inet_csk_*  │               │ ehash/lhash2/bhash  │
└─────────────┘               └─────────────────────┘
     ↓                                ↓
┌─────────────────────────────────────────────────────┐
│ IP 层（ip_input.c / ip_output.c）路由 / MTU 发现    │
└─────────────────────────────────────────────────────┘
                     ↓
                  [网卡]
```
...
| 子模块 | 主文件 | 角色 |
|---|---|---|
| Socket & INET 基础 | `net/ipv4/af_inet.c`, `include/net/inet_sock.h` | 协议族注册、socket 创建、4-tuple 绑定 |
| TCP 主控 + IPv4 粘合 | `net/ipv4/tcp.c`, `tcp_ipv4.c` | 中央状态机、连接 / 断开、TCP 选项解析 |
| 接收路径 | `net/ipv4/tcp_input.c` | 报文验证、序列号跟踪、ACK 处理、RTT 估算、乱序管理 |
| 发送路径 | `net/ipv4/tcp_output.c` | 队列管理、Nagle、GSO/TSO、重传队列、TCP 头构建 |
| 轻量 socket + TIME_WAIT | `net/ipv4/tcp_minisocks.c` | request_sock、TIME_WAIT 状态、graceful close |
| 计时器 | `net/ipv4/tcp_timer.c` | RTO 重传、零窗探针、keepalive、RTO 计算 |
| 拥塞控制框架 | `net/ipv4/tcp_cong.c` | 算法注册（CUBIC/BBR/Reno）+ 状态钩子 |
| 丢包检测 + 恢复 | `net/ipv4/tcp_input.c`, `tcp_recovery.c` | 三重 ACK、SACK、RACK 时基检测、F-RTO |
| 哈希表 + 查表 | `net/ipv4/inet_hashtables.c` | established/listen/bind hash + 早期 demux |
| 链路通知 | `net/ipv4/devinet.c` | NETDEV_DOWN/UP、multicast、ARP |

### 20.2 关键数据结构

- **`struct sock`**（`include/net/sock.h`）：所有网络 socket 的基础。核心字段 `sk_state`（12 个 TCP 状态值）、`sk_err / sk_err_soft`（ICMP/路由错误）、`sk_receive_queue / sk_write_queue`、`sk_prot`（指向 `tcp_prot`）、`sk_refcnt`。
- **`struct inet_connection_sock`**（`include/net/inet_connection_sock.h`）：扩展 `sock` 以支持面向连接协议。包含 `icsk_retransmit_timer / icsk_delack_timer`、`icsk_ca_ops`（指向 `struct tcp_congestion_ops`）、`icsk_rto`（当前 RTO，初值 ~200ms，范围 `[TCP_RTO_MIN=200μs, TCP_RTO_MAX=120s]`，参 `include/net/tcp.h:147-149`）、`icsk_retransmits`、`icsk_user_timeout`（`TCP_USER_TIMEOUT` socket option 值）。
- **`struct tcp_sock`**（`include/net/tcp.h`）：TCP 特有状态。`snd_una/snd_nxt/rcv_nxt`（序列号跟踪）、`snd_wnd/rcv_wnd`、`snd_wscale/rcv_wscale`、`srtt_us/mdev_us`（平滑 RTT 与均方差）、`snd_cwnd/snd_ssthresh`（拥塞窗口与慢启动阈值）、`tcp_rtx_queue`（红黑树，未确认报文）、`tcp_ofo_queue`（红黑树，乱序报文）。
- **`struct tcp_skb_cb`**（`include/net/tcp.h`）：每个 skb 的 TCP 控制块（48 字节，存于 `skb->cb[]`）。`seq/end_seq/ack_seq/sacked/eor`。
- **`struct request_sock`**（`include/net/request_sock.h`）：轻量 SYN 等待对象（~200B vs `tcp_sock` ~2KB+）。存初始 ISN/MSS/窗口选项，不分配数据路径缓冲；入 SYN backlog，三次握手完成时升级为完整 socket。
- **`struct inet_timewait_sock`**（`include/net/inet_timewait_sock.h`）：TIME_WAIT 专用轻量对象（~200B）。存重建 TCP 头的最小状态，确保乱序/重复 FIN-ACK 可被正确丢弃。默认驻留 `TCP_TIMEWAIT_LEN = 60s`（参 `include/net/tcp.h:129-131`）。

### 20.3 完整状态机与转移表

12 个状态定义于 `include/net/tcp_states.h:12-27`：

| # | 状态 | 角色 | 进入 | 退出 |
|---|---|---|---|---|
| 1 | `TCP_ESTABLISHED` | 双向数据流 | 三次握手完成 | close() / 收到 FIN |
| 2 | `TCP_SYN_SENT` | 客户端已发 SYN | `tcp_v4_connect()` | 收 SYN-ACK / RST / 超时 |
| 3 | `TCP_SYN_RECV` | 服务端已发 SYN-ACK | 服务端收 SYN | 收 ACK / 超时 |
| 4 | `TCP_FIN_WAIT1` | 已发 FIN，等 ACK | `close()` / `shutdown(WR)` | 收 ACK 或 FIN-ACK |
| 5 | `TCP_FIN_WAIT2` | 已收对 FIN 的 ACK，等对端 FIN | FIN_WAIT1 收到 ACK | 收 FIN（→TIME_WAIT）/ `tcp_fin_timeout=60s` 超时 |
| 6 | `TCP_TIME_WAIT` | 等 2×MSL，graceful close | 收 FIN-ACK 或 FIN_WAIT_2 超时 | 60s 后自动关闭，期间可重发 FIN-ACK |
| 7 | `TCP_CLOSE` | 完全关闭 | close() / 中止错误 | — |
| 8 | `TCP_CLOSE_WAIT` | 已收对端 FIN，等 close() | ESTABLISHED 收到 FIN | close() → LAST_ACK |
| 9 | `TCP_LAST_ACK` | 被动方已发 FIN | CLOSE_WAIT 调 close() | 收 ACK → CLOSED |
| 10 | `TCP_LISTEN` | 等入站 SYN | `listen()` | 收 SYN（创 request_sock）/ close() |
| 11 | `TCP_CLOSING` | 双方同时关闭，等对端 ACK | 发 FIN 后收 FIN（少见） | 收 ACK → CLOSED |
| 12 | `TCP_NEW_SYN_RECV` | SYN cookie 临时态 | backlog 满进 cookie 模式 | 收 cookie ACK 重建 / 超时 |

中央状态迁移：**`tcp_set_state(sk, new_state)`** `net/ipv4/tcp.c:2622-2690`。更新 `sk->sk_state`（via `inet_sk_state_store()` `:2688`）、维护 `TCP_MIB_CURRESTAB / ESTABRESETS`、转 `TCP_CLOSE` 时从 listen hash 移除并释放绑定端口、触发 BPF hook `BPF_SOCK_OPS_STATE_CB`（`:2658-2659`）。

### 20.4 发送路径详解

```
sys_sendmsg(2)
  → sock_sendmsg()                                  net/socket.c:702
    → tcp_sendmsg(sk, msg, size)                    tcp.c:1356-1394
      → tcp_sendmsg_locked(sk, msg, size)           tcp.c:1055-1350
        ① socket 状态/错误检查                     :1076-1115
        ② skb 分配 / 复用
        ③ 数据追加到 sk_write_queue                 :1200-1280
        ④ tcp_push() → tcp_write_xmit()
            tcp_write_xmit(sk, max_segs, nonagle, mss, GFP)
                                                    tcp_output.c:2533-2650
            ⓐ 抽取 write_queue 报文
            ⓑ Nagle 算法判定（NAGLE_OFF/CORK/PUSH）
            ⓒ GSO / TSO 合并
            ⓓ 对每报文 → tcp_transmit_skb()
                → 构建 TCP 头（端口/seq/ack_seq/win/flags/csum）
                → 追加 SACK 选项（按需）
                → icsk_af_ops->queue_xmit()      tcp_output.c:1413
            ⓔ 入重传队列：tcp_rbtree_insert(&sk->tcp_rtx_queue, skb)
            ⓕ 启动 RTO 计时器：
...
**Nagle**（RFC 896）：除非 `TCP_NAGLE_OFF` 或 `TCP_NAGLE_PUSH`，否则仅当所有已发数据被 ACK 后才允许发新小报文。**GSO/TSO**：把多个 TCP 报文打包成单个 skb 交给硬件分段，减少软件开销。

### 20.5 接收路径详解

```
netif_receive_skb()                               [网卡驱动]
  → ip_rcv()                                       ip_input.c:536
    → ip_local_deliver()                           ip_input.c:205
      → tcp_v4_rcv(skb)                            tcp_ipv4.c:1996-2084
        ① 早期 demux（无锁 RCU 读侧）
           __inet_lookup_skb(skb, sport, dport, iif, 0)
           若命中 established socket，跳过 full lookup
        ② Full lookup（上锁）
           __inet_lookup_established() 或 __inet_lookup_listener()
        ③ 状态分派
           ┌─────────────────────────────────────────────┐
           │ TCP_LISTEN     → tcp_v4_conn_request()      │ tcp_ipv4.c:1540
           │ TCP_SYN_SENT   → tcp_rcv_synsent_state_process() tcp_input.c:6397
           │ 其它           → tcp_v4_do_rcv()            │ tcp_ipv4.c:1711
           │                  → tcp_rcv_state_process()   │
           │                  / tcp_rcv_established()    │
           └─────────────────────────────────────────────┘
        ④ ESTABLISHED 快速路径：
           tcp_rcv_established()                  tcp_input.c:5920
           • 头预测（TCP 头 20B + 标志 ACK/PSH）→ 纳秒级处理
           • 纯 ACK 直接更新 snd_una/win
           • 顺序数据直入 sk_receive_queue
           • 乱序数据 → tcp_data_queue() → tcp_ofo_queue()
        ⑤ ACK 处理：
           tcp_ack(sk, skb, flag)                 tcp_input.c:3892-4130
           • 序号校验
           • 累积 ACK：tcp_clean_rtx_queue()
           • SACK：tcp_sacktag_write_queue()      tcp_input.c:1771-1813
           • RTT 采样 → tcp_rtt_estimator()       tcp_input.c:3637-3665
           • CC 回调：cong_avoid / in_ack_event / pkts_acked
        ⑥ 应用唤醒：
           sk_data_ready(sk)                       include/net/sock.h
           → epoll/poll/select 醒
```

### 20.6 三次握手详解

**客户端发起**（`tcp_v4_connect` `tcp_ipv4.c:209-340`）：

```c
:293   tcp_set_state(sk, TCP_SYN_SENT)
:294   inet_hash_connect()
:332   tcp_connect()                              tcp_output.c:4090-4151
         :4115 tcp_init_nondata_skb(buff, write_seq++, TCPHDR_SYN)
         :4120 tcp_rbtree_insert(&sk->tcp_rtx_queue, buff)
         :4124 tcp_transmit_skb()
         :4141 inet_csk_reset_xmit_timer(ICSK_TIME_RETRANS, rto, RTO_MAX)
```

RTO 初值 `TCP_TIMEOUT_INIT = 1s`（`include/net/tcp.h:153`）；指数退避 1/2/4/8/16/...s；上限 `TCP_RTO_MAX = 120s`；最多 `tcp_syn_retries = 6` 次（`:113-120`）；超限返 `ETIMEDOUT`。

**服务端响应** （`tcp_v4_conn_request` → `tcp_conn_request` `tcp_input.c:7160-7304`）：

```
:7183-7186  if (sk_acceptq_is_full || tcp_syncookies==2) → SYN cookie 模式
:7194       inet_reqsk_alloc(...) → 分配 request_sock（~200B）
:7223-7273  解析 SYN options（MSS / window scale / SACK / ts）
:7250-7258  生成 ISN（tcp_secure_seq）
:7295       inet_csk_reqsk_queue_hash_add() → 入 SYN backlog（红黑树）
:7303       af_ops->send_synack() → 发 SYN-ACK
            ★ Listener socket 仍 TCP_LISTEN，request_sock 在 backlog 等
```

**客户端收 SYN-ACK**（`tcp_rcv_synsent_state_process` `tcp_input.c:6397-6530`）：

```
:6411-6428  ACK 号 ∈ (ISS, SND.NXT] 校验
:6446-6451  if RST → tcp_reset()
:6460-6468  必须带 SYN，否则丢
:6475       tcp_ack() 处理内嵌 ACK
:6518       tcp_finish_connect()
              :6274  tcp_set_state(sk, TCP_ESTABLISHED)
              + tcp_init_congestion_control() / tcp_init_metrics()
:6523-6525  sk_state_change() 唤醒阻塞 reader
```

**服务端收最终 ACK**（`tcp_v4_do_rcv` → `tcp_check_req` → `tcp_v4_syn_recv_sock` `tcp_ipv4.c:1560-1650`）：

```
:1581       tcp_create_openreq_child() → 从 request_sock 模板分配完整 sock
:1588-1626  复制 SYN RX info
:1641       inet_ehash_nolisten(newsk) → 加入 established hash
:1644       tcp_move_syn() → 记录原 SYN 数据
            ★ newsk → TCP_ESTABLISHED，加入 accept 队列
```

**SYN cookies**（RFC 4987）：backlog 溢出时不分配 request_sock，把 ISN/MSS/窗口编码进 SYN-ACK 的 ACK number 字段；客户端回 ACK 时从 cookie 重建 state（无状态、抗 SYN flood）。代价：SACK 等高级选项无法协商。验证在 `cookie_v4_check()` (`net/ipv4/tcp_minisocks.c`)。

### 20.7 四次挥手与 TIME_WAIT

**主动方** (`tcp_close` `tcp.c:3083-3235`)：

```
tcp_close(sk)
  → tcp_close_state(sk)                          tcp.c:3237-3260
      ESTABLISHED → FIN_WAIT1   (发 FIN)
      CLOSE_WAIT  → LAST_ACK    (发 FIN)
      SYN_SENT    → CLOSED
      SYN_RECV    → FIN_WAIT1 / LAST_ACK
  → tcp_send_fin(sk)                             tcp_output.c:3268-3330
      • 分配 FIN skb
      • 入重传队列
      • 启动 RTO timer
```

**被动方**：收 FIN → `tcp_data_queue()` 标记 → 转 CLOSE_WAIT 发 ACK → 应用读完后 `close()` → `tcp_close()` → LAST_ACK。

**TIME_WAIT** (`tcp_time_wait` `tcp_minisocks.c:281-345`)：

```
tcp_time_wait(sk, state, timeo)
  :281  inet_twsk_alloc(sk, ...) → ~200B 轻量对象
  :297-305  copy: ISN / seq / ack / win / 时间戳
  :326  timeo = TCP_TIMEWAIT_LEN = 60s
  :333  inet_twsk_schedule(tw, timeo) → 启动 60s 定时器
  :337  inet_twsk_hashdash(tw, sk, ...) → 移到 TIME_WAIT hash
```

驻留 60s 期间收到重复 FIN：`tcp_timewait_state_process()` 校验序号匹配后回 ACK，不转移状态。**FIN_WAIT_2 超时**：`net.ipv4.tcp_fin_timeout` 默认 60s，防止对端永远不发 FIN 时 socket 永驻。

### 20.8 计时器子系统

每 socket 多个计时器，但通过 `icsk_pending` 复用单个 slot：

| ID | 用途 | 回调 |
|---|---|---|
| `ICSK_TIME_RETRANS` | RTO 重传 | `tcp_retransmit_timer()` `tcp_timer.c:512` |
| `ICSK_TIME_DACK` | 延迟 ACK | `tcp_delack_timer()` |
| `ICSK_TIME_PROBE0` | 零窗探针 | `tcp_probe_timer()` |
| `ICSK_TIME_LOSS_PROBE` | 尾部丢包 (RACK) | `tcp_probe_timer()` |
| `ICSK_TIME_REO_TIMEOUT` | 乱序重组等待 | `tcp_probe_timer()` |
| `ICSK_TIME_KEEPOPEN` | Keepalive | `tcp_keepalive_timer()`（独立启动） |

**`tcp_retransmit_timer()`** `tcp_timer.c:512-690`：
- 检查 `tcp_write_timeout()` (`:667`) 是否超过重试上限
- 未超：`icsk->icsk_backoff++`，指数退避 reschedule
- 超限：`tcp_write_err()` → `TCP_CLOSE` + 设 `ETIMEDOUT/EPIPE`

**`tcp_write_timeout()`** `tcp_timer.c:229-300`：
- SYN_SENT/SYN_RECV：检查 `icsk->icsk_syn_retries`（默认 6）
- ESTABLISHED：`tcp_retries1`（默认 3，~5min）触发黑洞检测/MTU probing；`tcp_retries2`（默认 15，~13-30min）最终 timeout
- 孤儿 socket：`tcp_orphan_retries()` 缩短重试，防资源泄露
- `TCP_USER_TIMEOUT` 覆盖：`:271` 检查 `icsk->icsk_user_timeout`，若设置则用其作为 wall-clock deadline

### 20.9 拥塞控制框架

可插拔，定义于 `tcp_cong.c` + `include/net/tcp.h:1092-1147`：
...
默认 **CUBIC**（`tcp_cubic.c`，高带宽长延迟数据中心场景）。其它：Reno、BBR（基于带宽估算）、Westwood、Vegas 等。状态机用 `enum tcp_ca_state`：**Open / Disorder / CWR / Recovery / Loss**。

接入点（`tcp_ack` `tcp_input.c:3892-4130`）：
- `:3928` `ca_ops->set_state()` 状态转移
- `:4070-4100` `ca_ops->cong_avoid(sk, ack, acked)` 更新 cwnd
- 三重 ACK 或 RTO → `ssthresh()` 减半 cwnd → recovery

### 20.10 丢包检测与恢复

- **三重 ACK 快速重传**：收到 3 个相同 ACK，无需等 RTO
- **SACK** (RFC 2018, `tcp_sacktag.c`)：通告非连续已收报文范围
  - `tcp_sacktag_write_queue()` `tcp_input.c:1771-1813` 标记 `TCPCB_SACKED_ACKED`
  - 计算 `tp->sacked_out`
- **RACK** (RFC 8985, `tcp_recovery.c`)：时基检测，记录最近 ACK 报文发送时间；新报文超过 reorder window（~1×RTT）未 SACK → 判丢
- **F-RTO**（forward RTO recovery）：判断 RTO 超时是否为虚假，避免不必要重传
- **DSACK**（duplicate SACK）：发送端检测虚假 fast retransmit
- **ECN** (RFC 3168)：路由器标记拥塞而非丢弃，TCP 据此降 cwnd

### 20.11 流量控制

- **接收窗口自适应** (`tcp_rcv_space_adjust()` `tcp_input.c:665-735`)：根据应用消费速率 + RTT 动态调整 `rcv_wnd`
- **发送缓冲自适应** (`tcp_sndbuf_expand()`)：根据 BDP = cwnd × RTT 自动扩 `sk_sndbuf`
- **窗口缩放** (RFC 7323)：`snd_wscale` / `rcv_wscale` 让窗口超过 64KB
- **零窗探针**：对端 `rcv_wnd=0` 时 `ICSK_TIME_PROBE0` 周期发 1B 探针获取窗口更新

### 20.12 ACK 处理详解

`tcp_ack()` `tcp_input.c:3892-4130` 中枢：

```
① 序列号校验 (:3900-3920)
② 累积 ACK：tcp_clean_rtx_queue() (:3944-3985)
   • 释放已 ACK 报文内存
   • RTT 采样 → tcp_rtt_estimator()
   • RTO 更新（tcp_set_rto）
③ SACK 处理：tcp_sacktag_write_queue() (:3987-3995)
④ CC 响应 (:4023-4120)
   • 进 CA_LOSS 时调 cong_avoid()
   • in_ack_event() 通知 ACK 到达
   • pkts_acked() 更新 pace/BDP
⑤ 窗口更新 (:3915-3925)
```

**RTT 估算** (`tcp_rtt_estimator()` `tcp_input.c:3637-3665`)：
- Karn 算法：忽略重传报文样本（避免歧义）
- SRTT：`srtt = 7/8 × srtt + 1/8 × sample`
- MDEV：`mdev = 3/4 × mdev + 1/4 × |sample - srtt|`
- RTO = `srtt + 4 × mdev`，clamp 到 `[TCP_RTO_MIN, TCP_RTO_MAX]`

### 20.13 哈希表与 Socket 查表

| 表 | 用途 | 键 | 查表 |
|---|---|---|---|
| **ehash** (Established) | 已建立 socket | 5-tuple `(sa,sp,da,dp,iif)` | `__inet_lookup_established()` |
| **lhash2** (Listen v2) | Listener 查表 | `(laddr, lport)` | `inet_lookup_listener_icsk()` |
| **bhash** (Bind) | 端口分配冲突 | `(port)` | `inet_csk_get_port()` |
| **TCP death row** | TIME_WAIT | 5-tuple | `inet_twsk_lookup()` |

**早期 demux**（无锁 RCU 路径，`__inet_lookup_skb()` `include/net/inet_hashtables.h`）：单 ACK 路径无 spinlock 竞争。

冲突链表用 `hlist_nulls_head`（不是普通 `hlist_head`），便于 RCU 读侧迭代时安全检测链表变化。

### 20.14 Netdev 集成与链路事件

**`inetdev_event()`** `net/ipv4/devinet.c:1558-1650`：netdev notifier callback。

`NETDEV_DOWN` 分支 `:1621-1623`：

```c
case NETDEV_DOWN:
    ip_mc_down(in_dev);   /* 仅通知 multicast */
    break;
```

**Linux 在本地接口 down 时不主动 abort TCP socket**——多宿主主机可能 failover，强制 abort 会破坏。

**被通知的**：路由表（FIB）、邻居表（ARP/NDISC）、`fib_inetaddr_notifier` 链。
**不被通知的**：TCP/UDP socket、应用层。

### 20.15 链路故障处理深度

让 socket 实际"死亡"的三条路径：

1. **重传上限** `tcp_write_timeout()` `tcp_timer.c:229-300`：
   - `tcp_retries1 = 3` (~5min) → black-hole detection / MTU probing
   - `tcp_retries2 = 15` (~13-30min) → `tcp_write_err()` → `TCP_CLOSE` + `ETIMEDOUT/EPIPE`
   - 对端真消失默认 ~20 分钟才检测到
2. **`TCP_USER_TIMEOUT` socket option**：覆盖 `tcp_retries2` 用 wall-clock ms 超时；`:271` 检查
3. **ICMP 路径错误** `tcp_v4_err()` `tcp_ipv4.c:484-634`：
   - `ICMP_DEST_UNREACH` (HOST/NET) / `ICMP_TIME_EXCEEDED` → `EHOSTUNREACH`
   - SYN_SENT/SYN_RECV (`:597-612`)：`tcp_done_with_error()` 立即 abort
   - ESTABLISHED (`:615-633`)：设 `sk_err`，下次 syscall 返错（RFC 1122）
   - `ICMP_FRAG_NEEDED`：不 abort，触发 PMTU 发现

**死对端检测**：`SO_KEEPALIVE` + `tcp_keepalive_time`（默认 7200s）+ `_intvl`（默认 75s）+ `_probes`（默认 9）。默认配置太松；生产场景常调到 `time=60s, intvl=10s, probes=5` 实现 ~1min 检测。

**孤儿 socket**：应用 close() 但 socket 还有未 ACK 数据时，进入 orphan 状态，重试上限缩短为 `tcp_orphan_retries()`（防止僵尸资源泄露）。

### 20.16 与本设计 v3.27 对照

| 维度 | Linux TCP | 本设计 v1 | v2 候选（§18） |
|---|---|---|---|
| **状态数** | 12 (含 NEW_SYN_RECV) | 10 (无 SYN cookie) | 10 + 事件驱动 |
| **Demux 关键** | 5-tuple `(sa,sp,da,dp,iif)` | `(session_id)` 单级 | 多设备需扩展 |
| **重传策略** | 指数退避，tcp_retries2=15，~20min 超时 | 指数退避，MAX_RETRY=11，~4s | TCP_USER_TIMEOUT 风格 (§18.7) |
| **RTT 估计** | Karn + SRTT/MDEV，RTO 自适应 | 暂无 (v1 单消息) | v2 可加 |
| **拥塞控制** | CUBIC/BBR/Reno 可插拔 | 无 (单消息握手) | 数据流时考虑 |
| **流量控制** | 窗口 + 缓冲自适应 | 无 (无数据路径) | 数据平面时补 |
| **丢包检测** | SACK/RACK/3-dupack/RTO | 重传上限 + 重试定时器 | UBcore 事件 (§18.9) |
| **本地 link down** | NETDEV_DOWN 不通知 socket | 同样不通知 | UBcore PORT_DOWN (§18.9) |
| **对端死掉检测** | tcp_retries2 ~20min 或 USER_TIMEOUT | MAX_RETRY ~4s 内置 | 多链路+事件可至毫秒 |
| **路径错误信号** | ICMP via tcp_v4_err | 无 | UBcore TP_ERR 等（§18.9） |
| **多 listen** | lhash2 支持百万级 | v1 单 listen | §18.2 |
| **两阶段 alloc** | request_sock (~200B) → full | 直接 alloc 完整 | 多 listen 时评估 |
| **TIME_WAIT** | 60s 驻留 | 4s (MSN window，非 2×MSL) | 多握手时重估 |

### 20.17 五条 TCP 工程经验映射

| TCP 经验 | 价值 | 本设计 v1 | v2 候选 |
|---|---|---|---|
| 两阶段 alloc | 防 SYN flood 内存耗尽 | 直接 alloc 完整 session | 多 listen 时考虑 |
| SYN cookie | 抗 SYN flood 终极兜底 (RFC 4987) | 单 listen 无 backlog | §18.2 |
| TCP_USER_TIMEOUT | 应用可控故障检测时间 | 硬编码 MAX_RETRY=11 | §18.7 |
| ICMP 快速信号 | 毫秒级故障检测 | 无 | §18.9 |
| 多哈希表分工 | 避免全局竞争 | 单 session_id 哈希 | 多 listen/service 时分层 |
...
_v3.14 修订（2026-05-14）相对 v3.13 的主要变化_：

第十三轮 reviewer 指出 v3.13 的修复在它**引入的内部释放**和**仍然调用 on_closed**之间留下了所有权歧义：

| # | 缺陷 | v3.14 修复 |
|---|------|-----------|
| 1 | **SYN_RCVD 失败时调 on_closed 与 v3.6 close-consume 语义冲突**（MEDIUM）：v3.13 在 SYN_RCVD 重传超限时既调 `on_closed(session, -ETIMEDOUT, ctx)` 又内部释放 alloc ref。但 `session->on_closed` 是从 listen session 复制来的——按 §6.5 文档化的被动关闭模式，listener 的 on_closed 典型实现是调 ubmad_wk_close(session)。而 v3.6 close-consume 即使在 invalid/CLOSED 状态也会消费一份 ref。结果：close consume + handler 内部 alloc-put = double-put + UAF。 | rt_work_handler 重传超限分支按 state 分两条：<br>**SYN_RCVD**：跳过 on_closed，仅 log + 内部释放 alloc ref + timer ref。注释说明为什么不能调 on_closed（borrowed pointer / UAF 风险）。<br>**其它三种 retry-relevant 状态**：保持 v3.13 行为，调 on_closed 通知（这些状态下 alloc ref 都由外部持有，调用方负责自己的 ref）。<br>新增 §18.8 列出"v1 没有 accept 失败回调"作为 v2 候选 API。 |

到 v3.14 为止，alloc-ref 在所有 retry-relevant 状态下的所有权和释放都明确：external owner 模型对 SYN_SENT / FIN_WAIT_1 / LAST_ACK 适用，internal release 对 SYN_RCVD 适用，且后者不再调可能误用 close 的 on_closed 回调。

_v3.15 修订（2026-05-14）相对 v3.14 的主要变化_：

第十四轮 reviewer 指出 v3.14 的修复并不完整——FIN_WAIT_1 和 LAST_ACK 状态下 retry 超限有**完全相同**的所有权问题，v3.14 只覆盖了 SYN_RCVD 一个状态。

| # | 缺陷 | v3.15 修复 |
|---|------|-----------|
| 1 | **FIN_WAIT_1 / LAST_ACK retry 超限同样调 on_closed → UAF**（HIGH）：v3.6 close-consume 让 ubmad_wk_close 在排完 FIN 重传 timer 之后消费调用方持有的 alloc ref。所以进入 FIN_WAIT_1 / LAST_ACK 状态后，session 上**只剩 timer ref**——上层调用方按 close-consume 契约不再持有 session 指针。但 v3.14 retry-limit 分支只对 SYN_RCVD 做了"不调 on_closed"，FIN_WAIT_1 / LAST_ACK 仍然落到 else 分支的 on_closed 调用。listener 通用 on_closed 实现按 §6.5 模式调 `ubmad_wk_close(session)`——给上层一个借用指针，listener 调用 close 又消费一份 ref，紧接着 handler 的 timer-ref put 就命中已释放内存。同样的 double-put + UAF。 | retry-limit 分支重构为显式 4 态 switch：<br>`SYN_SENT`：通知调用方失败（on_closed / complete），调用方持 alloc ref 自负释放。<br>`SYN_RCVD`：内部释放 alloc ref + 不通知（v3.14 行为保留）。<br>`FIN_WAIT_1` / `LAST_ACK`：**新增**——不通知（alloc ref 已被 close consume，没有外部持有者；通知会触发 UAF）；只 log + timer ref put。<br>`default`：WARN_ON_ONCE，防御未来新增状态忘了归类。 |
...
- **§16 第 13 项"SYN/SYN_ACK 重传工作：指数退避，超限则通知失败"**：同上，扩写成 v3.15 四态行为描述（顺便补上 v3.2 起就支持的 FIN 重传）。
...
_v3.19 修订（2026-05-14）的纯 prose 修正_：

第十七轮 reviewer 又找出三处描述与代码不一致——v3.16/v3.17 修描述时把 rt_work 范围写成 "FIN/FIN_ACK retransmit" 是过度声明（rt_work 实际只重传 SYN/SYN_ACK/FIN；FIN_ACK 是 process_wk_fin 在 CLOSE_WAIT/TIME_WAIT 收到重传 FIN 时反应式重发，不由本 timer 主动驱动）：

- **§2.x struct 注释**：`rt_work` 上方的注释还说 "SYN / SYN_ACK 重传定时器"。改成 SYN/SYN_ACK/FIN 并明确 FIN_ACK 不在此 timer 范围。
- **§7.2 retry-limit FIN_WAIT_1/LAST_ACK 分支注释 + log**：v3.15 注释和 pr_info 都说 "FIN/FIN_ACK 重传超限"。改成 "FIN 重传超限——也即对端始终没回 FIN_ACK"。
- **§16 表第 13 项**：v3.17 也说成 "SYN/SYN_ACK/FIN/FIN_ACK 重传"。改成 "SYN/SYN_ACK/FIN——FIN_ACK 不在此 timer 范围"。
...
_v3.23 修订（2026-05-14）_：

第二十一轮 reviewer 指出"错误一：`ubmad_post_send_wk` 返回 -EAGAIN" troubleshooting 还说"引入重试机制（在 `ubmad_wk_syn_rt_work_handler` 中重试 `ubmad_post_send_wk`）"——但 v3.20-v3.21 已经把三层自愈（caller pre-import / `with_import` 反应式包装 / rt_work import-on-EAGAIN 兜底）都加进去了。建议替换成"现有机制说明 + 真正不消时如何定位"。

更新后的"解决"段：
- 列出三种已存在的自愈机制及位置（§4.4/§5.4/§6.2 caller pre-import；§4.3 with_import；§7.2 rt_work import-on-EAGAIN）
- 真正卡 `-EAGAIN` 不消时根因通常是对端设备不在线（`ubmad_import_jetty` UBCM 超时返回 `-EHOSTUNREACH`）
- dmesg grep 模式："with_import: import failed" 或 "re-import tjetty failed"

无代码改动。

_v3.24 修订（2026-05-14）_：

新增 §19 评审过程回顾：把 v2 → v3.23 这 24 轮 review 的数字摘要、收敛性观察、几条结构化经验固化进文档。属于元内容；放在 §18 之后是因为两者都是关于本文档本身的反思（§18 列未决设计问题，§19 列已经解决的迭代过程的经验）。

无代码改动。第一次 review 收敛轮（round 23 零问题）后写下；如未来实施阶段又出新 bug，可以续记到 §19.5。

_v3.25 修订（2026-05-14）_：

基于内核源码探索（`/Volumes/KernelDev/kernel/`，OLK-6.6）补两节参考材料：

- **§18.9 [HIGH for v2]**：UBcore TP/Port 事件接入。已确认基础设施完备：`enum ubcore_event_type` 16 项含 `PORT_DOWN/TP_ERR/DEV_FATAL` (`include/ub/urma/ubcore_types.h:357-376`)；注册 API `ubcore_register_event_handler` (`ubcore_device.c:1358`)；workqueue dispatch 链路 (`:1397-1431`)。**Producer 已就位**：UDMA `udma_main.c:1065-1083` 在物理 link up/down 时触发 `PORT_ACTIVE/PORT_DOWN`。**Consumer 是空架子**：UBMAD 已注册 `dev_priv->handler` (`ub_mad.c:1124-1125`) 但 `ubmad_event_cb` 几乎是 no-op (`:134-146`)，PORT_DOWN 落到 `default` 当成"非法事件"丢掉。设计里给出两种集成方案（扩展现有 cb / 单独注册 handler），推荐前者；预期把 v1 的 ~4s `MAX_RETRY` 检测窗口降到微秒~毫秒级。
- **§20 参考：Linux TCP 三次握手与链路故障**：`net/ipv4/tcp_states.h:12-27` 12 个状态、`tcp_set_state()` 在 `tcp.c:2622`；客户端流程 `tcp_v4_connect → tcp_connect → tcp_rcv_synsent_state_process → tcp_finish_connect`（含 file:line）；服务端流程 `tcp_v4_conn_request → tcp_conn_request → request_sock 两阶段 alloc → tcp_v4_syn_recv_sock` 含 SYN cookies 兜底；链路故障**关键发现**：Linux 在本地接口 down 时 `inetdev_event` `:1621-1623` 只通知 multicast，不 abort socket，靠 `tcp_retries2`(默认 15) ~20min 超时或 `TCP_USER_TIMEOUT` 显式 deadline 让 socket 死掉；ICMP path errors 通过 `tcp_v4_err` `:484` 立即信号。最后给出五条 TCP 工程经验对应到本设计的 §18.x 候选改进。

无代码改动；纯参考材料新增。这两节也是给将来"看完 v3.x 想知道为什么这么设计 / 与 TCP 怎么对应"的读者提供锚点。

_v3.26 修订（2026-05-15）_：

第 24 轮 reviewer 在 v3.25 §18.9 上找出三项准确性问题（一个 MEDIUM、两个 LOW）：

| # | 缺陷 | v3.26 修复 |
|---|------|-----------|
| 1 | **MEDIUM：把 `udma_ctl.c:1075` 误标为 TP_ERR / TP_FLUSH_DONE 的 producer。** 实际上那段是 `to_hw_ae_event_type`——把 `UBCORE_EVENT_*` 翻译成 `UBASE_EVENT_TYPE_*` 给 AE auxiliary-info 查询用，是消费侧映射，不是 dispatch producer。 | 重写 §18.9 的 producer 表：实测全树 `ubcore_dispatch_async_event` 调用点只有 PORT_DOWN/ACTIVE (`udma_main.c:1081`)、ELR_DONE/ERR (`:1179`)、EID_CHANGE（4 处）。把没 producer 的事件（TP_*、DEV_FATAL、MIGRATE_VTP_*）单列为"dormant scaffolding"——同 §4.8/§4.9/§4.12 文档化的模式。同步把"两种集成方案"里的事件分支重排成"有 producer / 防御性"两类，把"预期检测延迟"小节按场景列表实际收益限定到 link/device 级故障（对端崩溃这类没 producer 的不能加速）。 |
| 2 | **LOW：enum 计数错。** v3.25 说 16 项，实际数 18 项。 | 改成 18 项；不展开列举（前文已列大部分）。 |
...
_v3.28 修订（2026-05-15）_：

§20 从 v3.25 的 ~150 行 6 子节（状态机 / 客户端 / 服务端 / 重传 + cookies / 链路故障 / 经验映射）扩展为 ~400 行 17 子节，做成完整的 Linux TCP 架构参考：

- §20.1 分层架构 ASCII 图 + 子模块速查表
- §20.2 关键数据结构（sock / inet_connection_sock / tcp_sock / tcp_skb_cb / request_sock / inet_timewait_sock）
- §20.3 完整 12 状态转移表 + tcp_set_state 中央函数
- §20.4 发送路径全调用链（sendmsg → write_xmit → transmit_skb，含 Nagle / GSO / RTO 启动）
- §20.5 接收路径全调用链（ip_rcv → tcp_v4_rcv → fast/slow path → tcp_ack → 应用唤醒），含早期 demux 和 header prediction
- §20.6 三次握手详解（v3.25 版本扩充，加入 RTO 计算 / SYN cookie 路径）
- §20.7 四次挥手 + TIME_WAIT 全流程（含 inet_twsk_alloc / 60s 驻留 / 重发 FIN-ACK）
- §20.8 计时器子系统（icsk_pending 复用槽 + 6 种 timer ID + tcp_write_timeout 决策树）
- §20.9 拥塞控制框架（tcp_congestion_ops + CUBIC/BBR/Reno + ca_state Open/Disorder/CWR/Recovery/Loss）
- §20.10 丢包检测与恢复（3-dupack / SACK / RACK / F-RTO / DSACK / ECN）
- §20.11 流量控制（rcv 窗口自适应 / sndbuf autotune / 窗口缩放 / 零窗探针）
- §20.12 ACK 处理详解（tcp_ack 五段处理 + Karn RTT 估算 + RTO 公式）
- §20.13 哈希表与查表（ehash/lhash2/bhash/death_row + RCU 无锁早期 demux）
- §20.14 netdev 集成（NETDEV_DOWN 只通知 multicast / FIB notifier）
- §20.15 链路故障深度（tcp_retries1/2 / TCP_USER_TIMEOUT / tcp_v4_err ICMP / SO_KEEPALIVE / orphan socket）
- §20.16 12 维度对照表（本设计 v1 vs Linux TCP vs v2 候选）
- §20.17 五条 TCP 工程经验映射到 §18.x 候选

无代码改动；扩展的纯参考材料。所有 file:line 引用换成 `net/ipv4/tcp.c:1234` 风格（不带 `/Volumes/KernelDev/kernel/` 前缀）方便实现者直接复制到 grep / vim。
