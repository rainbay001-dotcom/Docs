...
主动关闭方（A）                              被动关闭方（B）
  ...
  │                                           │  state: LAST_ACK
  │──[4] UBMAD_WK_FIN_ACK ─────────────────▶│
  │  state: TIME_WAIT (等 2×MSL)              │  state: CLOSED
  │      ↓
  │  state: CLOSED
```
...
ESTABLISHED ──close()──▶ FIN_WAIT_1 ──recv FIN_ACK──▶ FIN_WAIT_2 ──recv FIN──▶ TIME_WAIT ──2MSL──▶ CLOSED
ESTABLISHED ──recv FIN──▶ CLOSE_WAIT ──close()──▶ LAST_ACK ──recv FIN_ACK──▶ CLOSED
```

共 9 个状态：`CLOSED / LISTEN / SYN_SENT / SYN_RCVD / ESTABLISHED / FIN_WAIT_1 / FIN_WAIT_2 / CLOSE_WAIT / LAST_ACK / TIME_WAIT`
...
打开 `ub_mad.h`，找到 `enum ubmad_msg_type` 定义，大约在 `ub_mad.h:20-30`：

```c
enum ubmad_msg_type {
    UBMAD_UBC_CONN_REQ   = UBCORE_CM_CONN_REQ,   /* 与 UBCM 消息类型对齐 */
...
enum ubmad_wk_session_state {
    ...
    UBMAD_WK_TIME_WAIT   = 9,   /* 主动关闭方等待 2×MSL 后归还资源 */
};
...
/* TIME_WAIT 等待时长（毫秒），类比 TCP 2×MSL，这里取 4000ms */
 ...
 *   → 发送 FIN_ACK → 迁移到 TIME_WAIT → 启动 2×MSL 定时器
 *
