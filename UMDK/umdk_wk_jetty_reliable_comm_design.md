...
## 20. 参考：Linux TCP 三次握手与链路故障处理（v3.25 新增）

> 这一节是参考材料——把 Linux TCP（OLK-6.6 内核）的 3-way handshake 状态机、握手实现、和链路故障处理的关键 file:line 列出来，供本设计的实现者和评审者对照。本设计有意比 TCP 简化（单主机、不做 4-tuple demux、无拥塞控制），但 TCP 有几条工程经验是本设计可以学的（已在 §18.x 列出 v2 候选）。

### 20.1 TCP 状态机

定义在 `include/net/tcp_states.h:12-27`，**12 个状态**（比通常说的 11 个多一个 `TCP_NEW_SYN_RECV` 用于 SYN cookie 路径）：

| 状态 | 值 | 角色 |
|---|---|---|
| `TCP_ESTABLISHED` | 1 | 已连接，数据传输 |
| `TCP_SYN_SENT` | 2 | 客户端已发 SYN，等 SYN-ACK |
| `TCP_SYN_RECV` | 3 | 服务端已收 SYN，已发 SYN-ACK |
| `TCP_FIN_WAIT1` | 4 | 已发 FIN，等 ACK |
| `TCP_FIN_WAIT2` | 5 | 已收对 FIN 的 ACK，等对端 FIN |
| `TCP_TIME_WAIT` | 6 | 已收 FIN+ACK，graceful 关闭中 |
| `TCP_CLOSE` | 7 | 完全关闭 |
| `TCP_CLOSE_WAIT` | 8 | 已收对端 FIN，等本端 close() |
| `TCP_LAST_ACK` | 9 | 已发 FIN（被动方），等 ACK |
| `TCP_LISTEN` | 10 | 服务端等入站 SYN |
| `TCP_CLOSING` | 11 | 双方同时 close |
| `TCP_NEW_SYN_RECV` | 12 | SYN cookie 临时态 |

中央状态迁移函数 `tcp_set_state(sk, new_state)` 在 `net/ipv4/tcp.c:2622-2690`：更新 state、跑 BPF hook、维护 `TCP_MIB_CURRESTAB`/`ESTABRESETS` 计数、`→TCP_CLOSE` 时从 listen 哈希表 unhash，最后通过 `inet_sk_state_store()` 提交（`:2688`）。

**与本设计对照**：本设计 10 个状态（§3，CLOSED→TIME_WAIT），合并了 TCP 的 `CLOSING`（同时 close）；没有 SYN cookie 临时态。状态迁移分散在每个 process_wk_* 和 wk_close handler 内（state 字段直接赋值在 `session->lock` 内），没有集中的 set_state helper——因为状态数较少且每处迁移逻辑不同，集中化收益不明显。

### 20.2 客户端三次握手（SYN_SENT → ESTABLISHED）

```
tcp_v4_connect()  net/ipv4/tcp_ipv4.c:209
  :293   tcp_set_state(sk, TCP_SYN_SENT)
  :294   inet_hash_connect()                  ; 加入 TCP 哈希表（demux）
  :332   tcp_connect(sk)
           net/ipv4/tcp_output.c:4090
             :4115  tcp_init_nondata_skb(buff, write_seq++, TCPHDR_SYN)
             :4120  tcp_rbtree_insert(&sk->tcp_rtx_queue, buff)   ; 入重传队列
             :4124  tcp_transmit_skb()        ; 真实发送
             :4141  inet_csk_reset_xmit_timer(ICSK_TIME_RETRANS)  ; RTO 200ms-120s

         [等 SYN-ACK ...]

tcp_rcv_synsent_state_process()  net/ipv4/tcp_input.c:6397
  :6411-6428  validate ACK ∈ (ISS, SND.NXT]
  :6446-6451  if RST flag → tcp_reset()
  :6460-6468  ACK 必须带 SYN flag，否则丢
  :6471       tcp_ecn_rcv_synack()
  :6475       tcp_ack()                       ; 处理 SYN-ACK 内嵌的 ACK
  :6518       tcp_finish_connect()
                :6274  tcp_set_state(sk, TCP_ESTABLISHED)
                       初始化拥塞控制、keepalive
  :6523-6525  sk_state_change()               ; 唤醒阻塞的 readers
```

**与本设计对照**：客户端流程对应本设计 §4.4 `ubmad_wk_connect`。本设计简化点：
- 没有 `inet_hash_connect()`——session 单设备级，find_by_id 直接哈希查
- 没有重传队列管理（只有单个 `rt_work` 定时器；本设计每会话一条 in-flight 控制消息）
- 第三次 ACK 显式发送（§5.5 process_wk_syn_ack），TCP 通过 `tcp_ack()` 隐式发出

### 20.3 服务端三次握手（LISTEN → SYN_RECV → ESTABLISHED）

```
tcp_v4_rcv()  net/ipv4/tcp_ipv4.c    ; demux 入站包
  ↓ 命中 LISTEN socket
tcp_v4_conn_request()  :1540
  → tcp_conn_request()  net/ipv4/tcp_input.c:7160
       :7183-7186  if backlog 满或 syncookies=2 → 进 SYN cookie 模式
       :7194       inet_reqsk_alloc()         ; 分配轻量 request_sock（~200B，
                                                而非完整 sock ~2KB+）
       :7223-7273  parse SYN options、生成 ISN、init request 状态
       :7295       inet_csk_reqsk_queue_hash_add()   ; 入 SYN backlog
       :7303       af_ops->send_synack()      ; 发 SYN-ACK
                   ★ listener 仍然 TCP_LISTEN，request_sock 在 backlog 里等

         [等最终 ACK ...]

tcp_v4_do_rcv()  net/ipv4/tcp_ipv4.c:1711
  ↓ LISTEN socket 路径调 tcp_check_req() 在 backlog 找 matching req
  ↓
tcp_v4_syn_recv_sock()  :1560
  :1581  tcp_create_openreq_child(sk, req, skb)
                                                ; 从 request_sock 模板
                                                ; 分配完整 sock
  :1588-1626  copy SYN RX info（IP/port/window/options）
  :1641  inet_ehash_nolisten(newsk)             ; 移到 established 哈希
  :1644  tcp_move_syn()                         ; 记录原 SYN 数据
  ↑ newsk → TCP_ESTABLISHED 状态
  ↑ 返回到 accept 队列（应用 accept() 取走）
```

**与本设计对照**：服务端流程对应本设计 §5.4 process_wk_syn + §5.6 process_wk_ack。本设计**没有 request_sock 两阶段分配**——`process_wk_syn` 直接 `ubmad_alloc_session` 出完整对象。这是 §18.0 数据平面之外的另一个 v2 候选优化（"轻量 handshake 状态 → ACK 后 promote 到完整 session"）。

### 20.4 重传与 SYN cookies

- **客户端 SYN 重传**：`tcp_retransmit_timer()` `net/ipv4/tcp_timer.c:512`，指数退避 `icsk->icsk_backoff++`，最多 `tcp_syn_retries`（默认 6 次），超限 `tcp_write_err()` 返回 `ETIMEDOUT`。
- **服务端 SYN-ACK 重传**：`tcp_synack_retries`（默认 5 次）。
- **SYN cookies**（RFC 4987）：backlog 溢出时**不分配 request_sock**，把 ISN/MSS/window 编码进 SYN-ACK 的 ACK number 字段；客户端回 ACK 时服务端从 cookie 重建 state（无状态、抗 SYN flood）。代价：SACK 等高级选项无法协商。`tcp_conn_request()` 的 `:7264-7268` 分支 + `cookie_v4_check()` 在 `net/ipv4/tcp_minisocks.c`。

**与本设计对照**：本设计的重传策略（`UBMAD_WK_MAX_RETRY = 11`，`1 << rt_cnt` ms 退避，~4s 累计窗口）完全对得上 TCP 风格指数退避，只是上限远短——因为运行在受控 fabric。本设计**没有 SYN cookies** 兜底：backlog 概念在本设计就是单个 `dev_priv->listen_session`（v1 单 listen 限制），SYN 在握手未完成时落到 listen 上重复 SYN 通过 §3.8b `find_by_peer` 幂等处理；不存在 backlog 溢出。如果 v2 改成多 listen + 高并发（§18.2），需要重新评估 cookie-style 防御。

### 20.5 链路故障处理

**关键事实：Linux 在本地接口 down 时不主动 abort TCP socket。**

`inetdev_event()` `net/ipv4/devinet.c:1558`，`NETDEV_DOWN` 分支 `:1621-1623`：

```c
case NETDEV_DOWN:
    ip_mc_down(in_dev);    /* 仅通知 multicast 层 */
    break;
```

理由：多宿主主机可能有备用接口，强制 abort 会破坏 failover。

**实际让 socket "死亡" 的三条路径**：

1. **重传上限**——`tcp_write_timeout()` `net/ipv4/tcp_timer.c:229`：
   - `tcp_retries1 = 3`（≈5min）触发 black-hole detection / MTU probing。
   - `tcp_retries2 = 15`（≈20min）触发 `tcp_write_err()` 把 socket 推到 `TCP_CLOSE` 并返回 `ETIMEDOUT`/`EPIPE`。
   - **对端真消失 → 默认要 ~20 分钟才"知道"**。
2. **`TCP_USER_TIMEOUT` socket option**：覆盖 `tcp_retries2` 用应用指定的 wall-clock 超时（毫秒）；`:271` 检查。这是唯一让 deadline-aware app 可控的旋钮。
3. **ICMP path errors**——`tcp_v4_err()` `net/ipv4/tcp_ipv4.c:484`：
   - `ICMP_DEST_UNREACH` (HOST/NET) / `ICMP_TIME_EXCEEDED` → `EHOSTUNREACH`
   - `SYN_SENT`/`SYN_RECV` 状态 (`:597-612`)：`tcp_done_with_error()` 立即 abort
   - `ESTABLISHED` (`:615-633`)：设 `sk_err`，下次 syscall 返错（RFC 1122）
   - `ICMP_FRAG_NEEDED` 不 abort，触发 PMTU 发现

**与本设计对照**：

| 维度 | Linux TCP | 本设计 v1 | 本设计 v2 候选（§18.9） |
|---|---|---|---|
| 本地 link down 检测 | NETDEV_DOWN 不通知 socket（设计如此） | 同样不通知 | UBcore `PORT_DOWN` 事件可订阅 |
| 对端死掉检测 | `tcp_retries2`（~20min）或 `TCP_USER_TIMEOUT` | `MAX_RETRY` ~4s | 同 v1，外加事件触发 |
| 路径错误信号 | ICMP via `tcp_v4_err` | 无 | UBcore `TP_ERR` / `DEV_FATAL` 事件 |
| 用户可调超时 | `TCP_USER_TIMEOUT` | 硬编码 `MAX_RETRY=11` | §18.7 列待暴露为参数 |

详细的 UBcore 事件接入设计见 §18.9。本设计的 ~4s 检测窗口已经远好于 TCP 默认的 ~20min，但加上事件钩子能再降到微秒~毫秒级。

### 20.6 几条来自 TCP 的工程经验（已映射到 §18.x 候选）

| TCP 经验 | 本设计 v1 现状 | 对应 v2 候选 |
|---|---|---|
| 两阶段 alloc（request_sock → full sock） | 直接 alloc 完整 session | 未列；可在 §18.x 加 |
| SYN cookies 抗 backlog 溢出 | 单 listen 没有 backlog 概念 | §18.2（多 listen 时再评估） |
| `TCP_USER_TIMEOUT` 可调超时 | 硬编码 `UBMAD_WK_MAX_RETRY` | §18.7 |
| ICMP 风格的快速故障信号 | 无 | §18.9 |
| RST on `SYN_SENT` 立即 abort | SYN 重传到上限才放弃 | 未列；UB 是否有 RST 等价物待确认 |
