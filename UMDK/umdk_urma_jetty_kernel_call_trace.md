...
3. ~~UAPI ioctl boundary.~~ **Resolved in §4.10.** The actual layout is **three** parallel UAPI planes, not two:<br>**A.** URMA app path: char-dev ioctl on `/dev/uburma/<dev>` via `uburma_ioctl` (`uburma_cmd.c:4991`) → `g_uburma_cmd_handlers[]` (`uburma_cmd.c:4886`); `UBURMA_CMD_CREATE_JETTY` → `uburma_cmd_create_jetty` (`uburma_cmd.c:2368`) → `ubcore_create_jetty` (`ubcore_jetty.c:2118`); `UBURMA_CMD_IMPORT_JFR` → `uburma_cmd_import_jfr` (`uburma_cmd.c:3174`) → `ubcore_import_jfr`.<br>**B.** UVS topology snapshot path: char-dev ioctl on **`/dev/ubcore`** (single global node) via `ubcore_global_ioctl` (`ubcore_device.c:94`) gated on `cmd == UBCORE_UVS_CMD` (`_IOWR('V',1,...)` from `ubcore_uvs_cmd.h:24`) → `ubcore_uvs_global_cmd_parse` (`ubcore_uvs_cmd.c:391`) → `g_ubcore_uvs_global_cmd_funcs[]` (`:383`, 4 cmds: `SET_TOPO`/`GET_ROUTE_LIST`/`GET_TOPO`/`GET_PATH_SET`).<br>**C.** UVS general admin path: generic netlink family `ubcore_genl_family` (`ubcore_genl.c:134/169`) consuming `UBCORE_CMD_*` (the enum values are shared between B and C; only the unused `_IOWR('C',1,...)` macro at `ubcore_cmd.h:28` is vestigial).
