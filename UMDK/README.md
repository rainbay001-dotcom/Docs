...
- [`umdk_ub_vs_pcie_bringup.md`](umdk_ub_vs_pcie_bringup.md) — Side-by-side comparison of **UB device bringup** vs **PCIe probe**. Covers two-bus-type registration (`ub_bus_type` postcore_initcall + `ub_service_bus_type` module_init), the UBFI→UBUS→vendor→UBASE→upper-module chain, sysfs parallels (UB has `driver_override`, `config`, `resource` mmap exactly like PCIe), USI vs MSI/MSI-X, plus the runtime UBFM layer that PCIe has no analogue for. Includes a kernel-API translation table (`pci_register_driver` ↔ `register_ub_manage_subsystem_ops` / `auxiliary_driver_register`; `pci_alloc_irq_vectors_affinity` ↔ `ub_alloc_irq_vectors_affinity`; etc.) and a porting decision tree for PCIe-driver authors.
- [`umdk_user_kernel_boundary.md`](umdk_user_kernel_boundary.md) — Per-ioctl, per-struct catalogue of the URMA user↔kernel boundary. Four ioctl magics: `'U'` for main URMA (84 sub-commands across context / memory / token / JFR / JFS / JFC / jetty / TP / EID / device-query), `'E'` for event polling (3 ioctls all with `nr=0` differentiated by struct sizeof), `'V'` for UVS topology (TPSA), and one for UVS ubagg. Documents the `uburma_cmd_hdr` wrapper, the `uburma_cmd_udrv_priv` provider-passthrough blob, mmap follow-up patterns, the dual `CREATE/DELETE` vs `ALLOC/ACTIVE/DEACTIVE/FREE` lifecycle conventions, and verified counts (85 enum entries, max 25,600-byte args buffer).
...
- [`umdk_urma_perftest_function_graph.md`](umdk_urma_perftest_function_graph.md) — Source-level call graph for `urma_perftest`: top-level control flow, TCP metadata/barrier path, SIMPLEX vs DUPLEX resource setup, URMA API boundaries, latency/BW command dispatch, teardown, and a focused `send_lat --single_path` path.
- [`umdk_urma_well_known_jetty.md`](umdk_urma_well_known_jetty.md) — Public/well-known jetty reference: UDMA reserved `public_jetty` range, UBMAD fixed IDs 1/2, and the expanded UBCM link-setup call chain from `ubcore_exchange_tp_info()` through `ubmad_post_send()` / receive dispatch.
- [`umdk_ubmad_wk_jetty_deep_dive.md`](umdk_ubmad_wk_jetty_deep_dive.md) — Detailed UBMAD WK-jetty investigation: `ub_mad.c` resource model, exact creation timing, EID-add path, remote tjetty import, UBCM send/receive call graph, retry/duplicate handling, teardown, and `urma_perftest` relationship.
- [`umq_architecture.md`](umq_architecture.md) — Standalone UMQ deep-dive. Layered front-end → dlopen ops table → 4 real backends (UB / UB+ / IPC / UBMM(+)) + 3 stubs (IB / IB+ / UCP). Two API surfaces (base `enqueue/dequeue` vs pro `post/poll`), three qbuf pools (global TLS, huge, shm), two buffer modes (SPLIT / COMBINE), credit-based flow control, interrupt-vs-polling, async event types, TP-mode set, token-policy. Full file:line map with LOC for every backend, plus the `umq_buf_t` cache-line layout.
...
- [`umdk_urma_dispatcher_and_consumers.md`](umdk_urma_dispatcher_and_consumers.md) — Anatomy of the **URMA ioctl dispatcher** (`0xc0105501` decoded; 86 commands multiplexed onto one ioctl; the stack-local `urma_cmd_hdr_t` that confuses raw `sys_ioctl` traces; how to actually name an op from a trace), and a side-by-side of its three consumers: **perftest** (per-phase ioctl/TCP budget for `init_device → create_run_ctx`, `sock_sync_data` semantics, client/server symmetry, what's actually inside `register_seg`), **UMS** (`connect()` call chain through `clcsock` + CLC handshake + ubcore_create_jfc/jfr + ubcore_register_seg/import_seg), and **UMQ** (`umq_create` → `urma_create_jetty/jfc/jfr/register_seg/bind_jetty` ioctls). Closes with how UMS (kernel direct) and UMQ (userspace via ioctl) converge at the same `ubcore_*` surface. Synthesized from [`rainbay001-dotcom/UMDK#7`](https://github.com/rainbay001-dotcom/UMDK/issues/7).
- [`imp_interface.md`](imp_interface.md) — HiSilicon **IMP** (Integrated Management Processor) host↔firmware interface as it appears in the HNS3 NIC driver: CMDQ ring + mailbox + Vector0 IRQ bits (`IMP_RESET`, `IMP_CMDQ_ERR`, `IMP_RD_POISON`, `TRIGGER_IMP_RESET`), capability bits (`PHY_IMP_B`, `RAS_IMP_B`), ITCM ECC + RD poison error registers, debugfs/stats hooks. Includes the cross-product mapping to the UB equivalent **MUE** (Management UB Entity, spec §10.2.3) and notes how the same architectural slot recurs across Kunpeng / Ascend / HiNIC / Atlas.

**Spec deep-dive + academic papers + web research + open-question follow-ups:**
- [`umdk_spec_deep_dive.md`](umdk_spec_deep_dive.md) — Targeted reads of the **Chinese UB Base Spec 2.0** chapters that the English preview only had ToCs for: §6 Transport (24-bit PSN + 8M-1 send window, four retransmit algorithms, exp-backoff RTO defaults), §7 Transaction (slicing, ROI/ROT/ROL/UNO modes with SC dynamic alloc, Atomic with 9 sub-types incl. CAS/swap/store/load/fetch_*), **§8 Function Layer** (Jetty 4-state machine + 3 types + 3 group-policies, Public Jetty 2 = "Socket over UB", Ownership mechanism for shared-mem coherence, three URPC param-passing modes with 40 KB cutoff), §10 Resource Mgmt (UBFM Sub-Domain split, **MUE = Management UB Entity** definitive, 128-bit EID = 108-bit Prefix + 20-bit Sub ID, UPI partition with 15/24-bit lengths, USI Type 1 vs Type 2 interrupts, full slice taxonomy), §11 Security (3-flow device auth, **token rotation per §11.4.4 with worked timeline**, CIP with AES-GCM/SM4-GCM, TEE extension UTEI/HTEI/UTM/HTM model + 2-bit EE_bits → 4 address spaces), **Appendix H URPC Message Format** (48-bit Function ID layout, Status code table, Argument-DMA descriptor), and the **UVS naming resolution** (no canonical expansion exists; TPSA→UVS port history captured).
